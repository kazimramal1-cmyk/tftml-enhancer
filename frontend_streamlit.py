...
#  RAMAL KAZIMZADE | Şəkil + Fırça Silmə + Video
# ================================================================
...
# ── Fırça: PIL ilə şəkil üzərində rəngləmə ──────────────────────
def brush_tool(img_pil, key="brush"):
    """
    Streamlit-in file_uploader + PIL istifadə edərək fırça simulyasiyası.
    İstifadəçi koordinatları daxil edir, biz PIL ilə mask çəkirik.
    """
    iw, ih = img_pil.size
    ...
    # Session state — mask saxla
    if f"mask_strokes_{key}" not in st.session_state:
        st.session_state[f"mask_strokes_{key}"] = []

    html = f"""
<style>
#wrap_{key}{{text-align:center;user-select:none}}
#cv_{key}{{border-radius:12px;border:2px solid #4a1a8b;cursor:crosshair;
           display:block;margin:0 auto;max-width:100%;touch-action:none}}
.btns_{key}{{display:flex;gap:.6rem;justify-content:center;margin:.8rem 0;flex-wrap:wrap;align-items:center}}
.btns_{key} button{{border:none;border-radius:8px;cursor:pointer;font-size:.78rem;font-weight:700;padding:.45rem 1.1rem;color:#fff}}
.inf_{key}{{text-align:center;font-size:.72rem;color:#cc88ff;margin:.4rem 0;min-height:1.4em}}
</style>
<div id="wrap_{key}">
  <canvas id="cv_{key}" width="{cw}" height="{ch}"></canvas>
</div>
<div class="btns_{key}">
  <button onclick="setTool('brush')"
    style="background:linear-gradient(135deg,#8b0000,#cc2200)">🖌️ Fırça</button>
  <button onclick="setTool('erase')"
    style="background:linear-gradient(135deg,#1a3a6b,#2a5aab)">⬜ Pozucu</button>
  <button onclick="clearAll()"
    style="background:#222;border:1px solid #444;color:#aaa">🗑️ Hamısını Sil</button>
...
<div class="inf_{key}" id="inf_{key}">🖌️ Qırmızı fırça ilə silinəcək hissəni boyayın</div>
<input type="text" id="out_{key}" style="position:absolute;left:-9999px" value="">
...
(function(){{
  ...
  const bg=new Image(); bg.src='data:image/png;base64,{img_b64}';
  bg.onload=()=>draw();
  ...
  function paint(e){{
    ...
    draw();
    // stroku saxla
    const sx=Math.round(x*scX), sy=Math.round(y*scY);
    const strokes=JSON.parse(document.getElementById('out_{key}').value||'[]');
    strokes.push({{x:sx,y:sy,r:Math.round(sz/2*scX),t:tool}});
    document.getElementById('out_{key}').value=JSON.stringify(strokes);
    document.getElementById('inf_{key}').textContent='✅ Rəngləndi — "Fırça ilə Sil" düyməsini basın';
    document.getElementById('inf_{key}').style.color='#ff8888';
  }}
  ...
  window.setTool=t=>{{
    tool=t;
    document.getElementById('inf_{key}').textContent=
      t==='brush'?'🖌️ Fırça aktiv — silinəcək hissəni boyayın':'⬜ Pozucu aktiv — boyamı silin';
    document.getElementById('inf_{key}').style.color=t==='brush'?'#ff8888':'#88aaff';
  }};
  window.clearAll=()=>{{
    oc.clearRect(0,0,{cw},{ch}); draw();
    document.getElementById('out_{key}').value='';
    document.getElementById('inf_{key}').textContent='🖌️ Qırmızı fırça ilə silinəcək hissəni boyayın';
    document.getElementById('inf_{key}').style.color='#cc88ff';
  }};
}})();
</script>"""
    ...
    # Strokes-ları Streamlit-ə köçürmək üçün text_input istifadə edirik
    strokes_raw = st.text_input(
        "🖊️ Fırça məlumatı (avtomatik):",
        key=f"strokes_{key}",
        placeholder="Şəkil üzərini boyayın...",
        label_visibility="visible"
    )
    st.caption("💡 Şəkili boyayandan sonra bu sahə avtomatik dolur — sonra 'Sil' düyməsini basın.")
    return strokes_raw

def strokes_to_mask(strokes_raw, iw, ih):
    """Strokes JSON-dan PIL mask yarat"""
    mask = Image.new("L", (iw, ih), 0)
    draw = ImageDraw.Draw(mask)
    try:
        strokes = json.loads(strokes_raw)
        for s in strokes:
            x,y,r = s["x"],s["y"],s["r"]
...
<div class="ssub">AI Şəkil · Fırça Silmə · Video | Real-ESRGAN 4×</div>
""", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════
#  TAB 2 — FIRÇA İLƏ SİLMƏ
# ══════════════════════════════════════════════════════════════════
with tab2:
    st.markdown("""<div class="brush-tip">
    🖌️ <b>Fırça ilə Silmə — necə işləyir:</b><br>
    1️⃣ Şəkil yükləyin<br>
    2️⃣ <b style="color:#ff8888">🖌️ Fırça</b> ilə silinəcək hissəni <b>qırmızı</b> boyayın<br>
    3️⃣ <b style="color:#88aaff">⬜ Pozucu</b> ilə səhvi düzəldin<br>
    4️⃣ Strokes sahəsinə məlumat gəldikdən sonra <b>"Fırça ilə Sil"</b> basın
    ...
    if brush_file:
        ...
        if btn_brush:
            if not strokes_raw or strokes_raw.strip() in ("", "[]"):
                st.warning("⚠️ Əvvəlcə şəkil üzərini boyayın — sonra bu düyməni basın!")
            else:
                # PIL ilə mask yarat
