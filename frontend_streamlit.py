# ================================================================
#  frontend_streamlit.py — TFTML ENHANCER AI v8
...
import requests, base64, hashlib, io, time, threading, os
...
#  KONFİQURASİYA
# ══════════════════════════════════════════════════════
API_URL   = "https://stacie-apertural-ardelia.ngrok-free.dev"  # ← Colab URL-ini buraya yaz
LOGO_FILE = "logo_png.jpeg"   # faylın adı tam belə olmalıdır
...
.block-container { max-width: 960px !important; padding: 1.5rem !important; margin: 0 auto !important; }
.stApp::before {
...
.logo-emoji {
    width: 110px; height: 110px; border-radius: 50%; background: #1a6b2f;
    display: flex; align-items: center; justify-content: center;
    font-size: 3rem; margin: auto; animation: glowPulse 3s ease-in-out infinite;
}
@keyframes glowPulse {
...
.status-ok  { background: linear-gradient(135deg,#0a1f10,#0d2b15); border: 1px solid #1a6b2f; border-radius: 12px; padding: .55rem 1.2rem; font-size: .78rem; color: #4dff88; font-weight: 600; margin-bottom: 1rem; }
.status-err { background: linear-gradient(135deg,#1f0a0a,#2b0d0d); border: 1px solid #6b1a1a; border-radius: 12px; padding: .55rem 1.2rem; font-size: .78rem; color: #ff6b6b; font-weight: 600; margin-bottom: 1rem; }
[data-testid="stTabs"] [data-testid="stTab"] {
    background: transparent !important; border: none !important;
    color: #666 !important; font-weight: 600 !important; font-size: .85rem !important;
    padding: .6rem 1.2rem !important; border-radius: 10px 10px 0 0 !important;
}
...
[data-testid="stTabContent"] {
    border: 1.5px solid #1e251e !important; border-radius: 0 16px 16px 16px !important;
...
.badge { display: inline-block; font-size: .58rem; font-weight: 700; letter-spacing: .1em; text-transform: uppercase; padding: .2rem .55rem; border-radius: 4px; margin-bottom: .4rem; }
...
.inpaint-tip { background: linear-gradient(135deg,#1a1200,#2b1e00); border: 1px solid #6b4e00; border-radius: 12px; padding: .8rem 1.2rem; font-size: .78rem; color: #ffcc44; line-height: 1.7; margin: .6rem 0; }
.video-warn  { background: linear-gradient(135deg,#1a1200,#2b1e00); border: 1px solid #6b4e00; border-radius: 14px; padding: 1rem 1.4rem; font-size: .82rem; color: #ffcc44; font-weight: 600; margin: 1rem 0; text-align: center; }
...
def load_logo_b64(filename):
    """logo_png.jpeg faylını axtarır, tapılmasa None qaytarır"""
    try:
...
LOGO_B64 = load_logo_b64(LOGO_FILE)
...
def composite_bg(fg, bg):
    try:
        ...
        st.warning("⚠️ rembg quraşdırılmayıb")
        return fg
...
if LOGO_B64:
    st.markdown(
        ...
        f'<img class="logo-img" src="data:image/jpeg;base64,{LOGO_B64}">'
        f'</div>',
        unsafe_allow_html=True)
else:
    # Logo tapılmasa xəbərdarlıq ver, dayanma
    st.warning(f"⚠️ Logo faylı tapılmadı: `{LOGO_FILE}` — tətbiq işləyir")
    st.markdown(
        '<div class="logo-wrap"><div class="logo-emoji">🎓</div></div>',
        unsafe_allow_html=True)
...
if api_ok:
    st.markdown('<div class="status-ok">✅ Colab Backend — Online · Aktiv</div>',
                unsafe_allow_html=True)
else:
...
#  TABLƏR
# ══════════════════════════════════════════════════════
tab1, tab2, tab3 = st.tabs(["🖼️  Şəkil Artır", "🧹  Obyekt Sil", "🎬  Video Artır"])
...
with tab1:
    ...
    if uploaded:
        ...
        with col_prev:
            ...
            if apply_bg:
                bg_src    = custom_bg if BACKGROUNDS[bg_choice]=="custom" else fetch_bg(BACKGROUNDS[bg_choice])
                final_img = composite_bg(edited, bg_src) if bg_src else edited
            else:
                final_img = edited
    ...
    if btn1 and final_img:
        ...
        def spin():
            ...
            while not stop[0]:
                msg_box.markdown(
                    ...
                    f'color:#e07020;padding:.7rem">{MSGS[i%len(MSGS)]}</div>',
                    unsafe_allow_html=True)
                time.sleep(2); i += 1
        ...
        try:
            ...
            data = resp.json()
            if data.get("success"):
                rb = base64.b64decode(data["image"])
            else:
...
with tab2:
    st.markdown(
        '<div class="inpaint-tip">🖌️ Şəkil yükləyin → Silmək istədiyiniz obyektin '
        'üzərini <b style="color:#e07020">narıncı fırça</b> ilə rəngləyin → '
        '<b>"Obyekti Yox Et"</b> düyməsini basın.</div>',
        unsafe_allow_html=True)
    ...
    if inp_file:
        ...
        try:
            ...
            with col_ctrl:
                stroke_w = st.slider("Fırça", 5, 60, 20, key="sw")
            with col_c:
            ...
            if btn_inp and canvas_result is not None \
                        and canvas_result.image_data is not None:
                mask_rgba = Image.fromarray(
                    canvas_result.image_data.astype(np.uint8), "RGBA")
                _, _, _, a = mask_rgba.split()
                mask_bin  = a.point(lambda x: 255 if x > 10 else 0)\
                             .resize((iw, ih), Image.NEAREST)
                mask_rgb  = Image.merge("RGB", [mask_bin]*3)

                prog2 = st.progress(0, "Göndərilir...")
                try:
                    resp = requests.post(
                        f"{API_URL}/sam-inpaint",
                        files={"image": ("image.png", pil_to_bytes(inp_pil), "image/png"),
                               "mask":  ("mask.png",  pil_to_bytes(mask_rgb), "image/png")},
                        timeout=300,
                        headers={"bypass-tunnel-reminder": "yes",
                                 "ngrok-skip-browser-warning": "true"})
                    ...
                    if data.get("success"):
                        ...
                        with c2:
                            st.markdown('<p style="text-align:center"><span class="badge b-enh">OBYEKTSİZ</span></p>',
                                        unsafe_allow_html=True)
            ...
            st.info("📦 `streamlit-drawable-canvas` requirements.txt-ə əlavə edin")

# ── TAB 3 — VİDEO ────────────────────────────────────
with tab3:
    st.markdown(
        '<div class="video-warn">⚠️ Video emalı bir neçə dəqiqə çəkə bilər.<br>'
        'Pəncərəni bağlamayın!</div>',
        unsafe_allow_html=True)
    ...
    if video_file:
        st.video(video_file)
        st.caption(f"📦 {video_file.size/1024/1024:.1f} MB · {video_file.name}")
    ...
    if btn3 and video_file:
        ...
        mv = ["🎬 Kadrlar ayrılır...", "⚡ GPU hər kadrı emal edir...",
              "🔄 Yenidən yığılır...", "✨ Möcüzə baş verir..."]
        ...
        try:
            ...
            if data3.get("success"):
                rb3 = base64.b64decode(data3["image"])
            else:
                err3 = data3.get("error", "Naməlum xəta")
        except Exception as e:
            err3 = str(e)
        ...
        if err3:
            ...
            st.balloons()
            st.success(f"🎉 Hazır! {data3.get('original','?')} → "
                       f"{data3.get('enhanced','?')} | {data3.get('frames','?')} kadr")
