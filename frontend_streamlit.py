...
#  RAMAL KAZIMZADE | Python 3.14 uyğun | Stabil versiya
# ================================================================
...
def safe_post(url, timeout=300, **kwargs):
    try:
        ...
        return None, f"{type(e)._name_}: {str(e)}"
...
with tab1:
    ...
    if up1:
        ...
        with fx:
            ...
            w,h = op.size
            a,b = st.columns(2)
            with a: cl=st.number_input("Sol",0,w-10,0,step=5,key="cl"); ct=st.number_input("Yuxarı",0,h-10,0,step=5,key="ct")
            with b: cr=st.number_input("Sağ",10,w,w,step=5,key="cr"); cb=st.number_input("Aşağı",10,h,h,step=5,key="cb")
            ...
            if BACKGROUNDS[bgc]=="custom":
                cf=st.file_uploader("Öz fonunuzu yükləyin",type=["jpg","jpeg","png","webp"],key="cf",label_visibility="visible")
    ...
    b1=st.button("✨  AI ilə 4× Keyfiyyəti Artır",disabled=not(up1 and api_ok and fim is not None),key="b1")
    if b1 and fim:
        sb=pil_to_bytes(fim); hh=hashlib.md5(sb).hexdigest()
        pg=st.progress(0); mb=st.empty(); st_=[False]
        ...
        st_[0]=True; mb.empty(); pg.progress(100,"Hazır! 🎉")
        if err: st.error(f"❌ {err}")
            ...
            c1,c2=st.columns(2)
            with c1: st.markdown('<p style="text-align:center"><span class="badge b-orig">REDAKTƏLİ</span></p>',unsafe_allow_html=True); st.image(fim,use_container_width=True)
            ...
            d1,d2=st.columns(2)
            with d1: st.download_button("⬇  Artırılmışı Endir",rb,f"enhanced_{up1.name.rsplit('.',1)[0]}.png","image/png",use_container_width=True)
            with d2: st.download_button("⬇  Redaktəlini Endir",sb,f"edited_{up1.name.rsplit('.',1)[0]}.png","image/png",use_container_width=True)
            st.markdown('</div>',unsafe_allow_html=True)
...
with tab2:
    ...
    up2=st.file_uploader("📸  Şəkil seçin (SAM üçün)",type=["jpg","jpeg","png","webp"],key="u2")
    if up2:
        ip=Image.open(up2).convert("RGB")
        iw,ih=ip.size
        sam_canvas(ip, key="s1")
        cj=st.text_input("📍 Koordinatlar:",placeholder='[{"x":200,"y":150,"label":1}]',
        ...
        if bs:
            cks=[]
            try: cks=json.loads(cj) if cj.strip() else []
            except: st.warning("⚠️ JSON oxunmadı")
            if not cks:
                cks=[{"x":iw//2,"y":ih//2,"label":1}]
                st.info(f"ℹ️  Mərkəz nöqtəsi istifadə edilir ({iw//2},{ih//2})")
            ob=pil_to_bytes(ip); pg2=st.progress(0,"SAM-a göndərilir...")
            try:
                ...
                try: data=resp.json()
                except: pg2.progress(100,"Xəta!"); st.error(f"❌ JSON parse xətası: {resp.text[:200]}"); data=None
                if data:
                    if data.get("success"):
                        pg2.progress(100,"Hazır! 🎉")
                        st.success(f"🎉 Tamamlandı! {'SAM ✅' if data.get('sam_used') else 'Fallback'}")
                        c1,c2,c3=st.columns(3)
                        with c1: st.markdown('<p style="text-align:center"><span class="badge b-orig">ORİGİNAL</span></p>',unsafe_allow_html=True); st.image(ip,use_container_width=True)
                        with c2:
                            if data.get("mask"):
                        ...
                        with c3: st.markdown('<p style="text-align:center"><span class="badge b-enh">NƏTİCƏ</span></p>',unsafe_allow_html=True); st.image(Image.open(io.BytesIO(base64.b64decode(data["image"]))),use_container_width=True)
                        st.download_button("⬇  Nəticəni Endir",base64.b64decode(data["image"]),f"sam_{up2.name.rsplit('.',1)[0]}.png","image/png",use_container_width=True)
                    else: pg2.progress(100,"Xəta!"); st.error(f"❌ {data.get('error','Naməlum xəta')}")
            except requests.exceptions.Timeout: pg2.progress(100,"Xəta!"); st.error("❌ Timeout (180s)")
            except requests.exceptions.ConnectionError: pg2.progress(100,"Xəta!"); st.error("❌ Backend əlçatan deyil")
            except Exception as e: pg2.progress(100,"Xəta!"); st.error(f"❌ {type(e)._name_}: {e}")
...
with tab3:
    ...
    if up3:
        st.video(up3)
        st.markdown(f'<div style="font-size:.76rem;color:#666;padding:.3rem 0">🎬 <span style="color:#bbb">{up3.name}</span> &nbsp; 📦 <span style="color:#bbb">{up3.size/1024/1024:.1f} MB</span></div>',unsafe_allow_html=True)
    b3=st.button("🎬  Video 4× Keyfiyyətini Artır",disabled=not(up3 and api_ok),key="b3")
    if b3 and up3:
        vb=up3.read()
        ...
        def vsp():
            i=0
            while not s3[0]: m3.markdown(f'<div class="spin-msg">{vm[i%4]}</div>',unsafe_allow_html=True); time.sleep(3); i+=1
        threading.Thread(target=vsp,daemon=True).start()
        p3.progress(10,"Video göndərilir...")
        rb3,mt3,er3=enhance_video_cached(vb,up3.name,API_URL)
        s3[0]=True; m3.empty()
        if er3: p3.progress(100,"Xəta!"); st.error(f"❌ {er3}")
        else:
            p3.progress(100,"Video hazır! 🎉"); st.balloons()
            st.success(f"🎉 {mt3.get('original','?')} → {mt3.get('enhanced','?')} | {mt3.get('frames','?')} kadr")
            st.download_button("⬇  4× Videonu Endir",rb3,f"enhanced_{up3.name.rsplit('.',1)[0]}.mp4","video/mp4",use_container_width=True)
