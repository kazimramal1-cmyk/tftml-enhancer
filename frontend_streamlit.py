...
# ══════════════════════════════════════════════════════
#  KONFİQURASİYA — BU SƏTRİ DƏYİŞ
# ══════════════════════════════════════════════════════
API_URL  = "https://stacie-apertural-ardelia.ngrok-free.dev"
LOGO_B64 = ""  # boş burax, emoji logo göstəriləcək
...
if api_ok:
    st.markdown(
        '<div class="status-ok">✅ Colab Backend — Online · Aktiv</div>',
        unsafe_allow_html=True)
else:
    st.markdown(
...
with tab1:
        ...
        with col_fx:
            ...
            with st.expander("🖼️ Arxa Fon", expanded=False):
                ...
                if BACKGROUNDS[bg_choice] == "custom":
                    ...
                    bg_img = fetch_bg(BACKGROUNDS[bg_choice])
                    if bg_img:
                        st.image(bg_img.convert("RGB").resize((220,124), Image.LANCZOS),
    ...
    if btn1 and final_img:
        ...
        def spin():
            ...
            while not stop[0]:
                msg_box.markdown(
                    f'<div style="text-align:center;font-size:.95rem;'
                    f'font-weight:600;color:#e07020;padding:.7rem">'
                    f'{MSGS[i % len(MSGS)]}</div>',
                    unsafe_allow_html=True)
                time.sleep(2); i += 1
        ...
        if err:
            ...
            with c1:
                st.markdown('<p style="text-align:center">'
                            '<span class="badge b-orig">REDAKTƏLİ</span></p>',
                            unsafe_allow_html=True)
                st.image(final_img, use_container_width=True)
                st.caption(f"📐 {final_img.width}×{final_img.height} px")
            with c2:
                st.markdown('<p style="text-align:center">'
                            '<span class="badge b-enh">4× AI</span></p>',
                            unsafe_allow_html=True)
...
with tab2:
    ...
    if inp_file:
        ...
        try:
            ...
            if btn_inp and canvas_result is not None \
                ...
                try:
                    resp = requests.post(
                        ...
                        files={
                            "image": ("image.png", pil_to_bytes(inp_pil), "image/png"),
            ...
            st.info("📦 Zəhmət olmasa requirements.txt-ə əlavə edin:\n\n"
