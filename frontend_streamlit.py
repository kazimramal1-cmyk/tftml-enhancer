# ================================================================
#  frontend_streamlit.py — TFTML ENHANCER AI v3
#  Slider müqayisə + Batch upload + Yeni drag&drop
# ================================================================
...
.block-container{max-width:900px!important;padding:2rem 1.5rem!important;margin:0 auto!important}
...
.card{background:linear-gradient(145deg,#131613,#111311);border-radius:20px;
  border:1.5px solid #1e251e;padding:1.8rem;margin-bottom:1.4rem;
  box-shadow:0 4px 40px rgba(0,0,0,.5)}

/* Upload zona */
[data-testid="stFileUploader"]{
  border:2px dashed transparent!important;border-radius:18px!important;
...
[data-testid="stFileUploader"]:hover{
  ...
  box-shadow:0 0 25px rgba(26,107,47,.2),0 0 50px rgba(224,112,32,.1)!important}
...
.batch-item-name{font-size:.8rem;color:#bbb;font-weight:600;margin-bottom:.5rem}
.batch-progress{height:4px;border-radius:2px;background:#1e251e;margin:.4rem 0;overflow:hidden}
.batch-bar{height:100%;background:linear-gradient(90deg,#1a6b2f,#e07020);
...
def enhance_cached(img_bytes: bytes, api_url: str):
    try:
        ...
        return None, data.get("error", "Naməlum xəta")
...
# ── Rejim seçimi ─────────────────────────────────────────────────
mode = st.radio("", ["🖼️  Tək Şəkil", "📦  Toplu (Batch)"],
...
if "Tək" in mode:
    ...
    if uploaded:
        ...
        with c2:
            h = hashlib.md5(uploaded.read()).hexdigest()[:8]; uploaded.seek(0)
            st.markdown(f"""<div style="padding:.6rem 0;font-size:.82rem;color:#777;line-height:2.2">
            ...
            🔑 <code style="font-size:.7rem;background:#0d0f0e;color:#4dff88;
                padding:.15rem .45rem;border-radius:4px;border:1px solid #1a6b2f">{h}</code>
    ...
    if btn and uploaded:
        ...
        if err:
            ...
            st.markdown("""<div style="display:flex;align-items:center;
              ...
              <span style="font-family:'Playfair Display',serif;font-size:1rem;color:#eee">
...
# ══════════════════════════════════════════════════════════════════
    ...
    if uploads:
        ...
        ✅ {len(uploads)} fayl seçildi</div>""", unsafe_allow_html=True)

    btn_batch = st.button(f"✨  {len(uploads) if uploads else 0} Şəkli Emal Et",
                          disabled=not (uploads and api_ok))
    st.markdown('</div>', unsafe_allow_html=True)

    if btn_batch and uploads:
        st.markdown(f'<div class="card">', unsafe_allow_html=True)
        st.markdown("""<span style="font-family:'Playfair Display',serif;
          font-size:1rem;color:#eee">Batch Emal Nəticələri</span>""",
          unsafe_allow_html=True)

        total_prog = st.progress(0, f"0 / {len(uploads)}")
        ...
        for idx, up in enumerate(uploads):
            ...
            with st.expander(f"🖼️  {up.name}  ({up.size/1024/1024:.2f} MB)", expanded=True):
                ...
                with c2:
                    spin_ph = st.empty()
                    spin_ph.markdown('<div class="spin-msg">⚡ Emal edilir...</div>',
                                    unsafe_allow_html=True)
                    result_bytes, err = enhance_cached(img_bytes, API_URL)
                    spin_ph.empty()
            ...
            total_prog.progress((idx+1)/len(uploads),
                                f"{idx+1} / {len(uploads)} tamamlandı")
